<?php
	...
	class eciTwitchApi {
		...
		private function _logUserInWithTwitch( $apiUserInfo ) {
			...
			$userInfoWithId = getRowWithValue( 'user', 'twitch_user_id', $apiUserInfo['id'] );

			// check for user with email
			$userInfoWithEmail = getRowWithValue( 'user', 'email', $apiUserInfo['email'] );
			...
			if ( $userInfoWithId || ( $userInfoWithEmail && !$userInfoWithEmail['password'] ) ) { // user was found by email/id log them in
				...
				updateRow( 'user', 'twitch_user_id', $apiUserInfo['id'], $userId );
				updateRow( 'user', 'twitch_access_token', $this->_accessToken, $userId );
				updateRow( 'user', 'twitch_refresh_token', $this->_refreshToken, $userId );

				// get user info
				$userInfo = getRowWithValue( 'user', 'id', $userId );
				...
				$signupUserInfo = array( // data we need to insert the user in our database
					...
					'user_name' => $apiUserInfo['display_name'], // using display_name as first name cause not first name in Twitch response
